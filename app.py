    ...
    if data['filtervideo'] != 0:
        ...
        if model_type == 'both':
            scores_clip, list_clip_ids, _, _ = CosineFaiss.text_search(
                query, index=None, k=k, model_type='clip', batch=batch)
            scores_blip, list_blip_ids, _, _ = CosineFaiss.text_search(
                query, index=None, k=k, model_type='blip', batch=batch)
            ...
            lst_scores, list_ids, _, list_image_paths = CosineFaiss.text_search(query, index=None, k=k, model_type=model_type, batch=batch)
