...
def search_by_filter(prev_result, text_query, k, mode, model_type, range_filter, ignore_index, keep_index, Sceneid2info, DictImagePath, CosineFaiss, KeyframesMapper):
    ...
    if ignore_index is not None:
        ...
        for idx in ignore_index:
            ...
            data_part, video_id, frame_id = image_path.replace(
                '/data/KeyFrames/', '').replace('.webp', '').split('/')
            key = f'{data_part}_{video_id}'.replace('_extra', '')
    ...
    for item in prev_result:
        ...
        for idx in lst_idxs:
            ...
            if mode == 1:
                start, end = int(scene_idx[3]) + \
                    1, int(scene_idx[3])+range_filter
            else:
                start, end = int(scene_idx[3]) - \
                    range_filter, int(scene_idx[3])-1
    ...
    for i, image_path in enumerate(list_image_paths):
        ...
        result_dict[key]['video_info']['lst_keyframe_idxs'].append(
            int(frame_id))
        result_dict[key]['video_info']['lst_scores'].append(
            float(lst_scores[i]))
    ...
    result = sorted(result, key=lambda x: x['video_info']['lst_scores']
                    [0] + x['video_info_prev']['lst_scores'][0], reverse=True)
