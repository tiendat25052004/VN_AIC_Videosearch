            ...
            if (not os.path.exists(os.path.join(save_tfids_object_path, f'tfidf_transform_{data_type}.pkl'))):
                if context_data == None:
                    ...
                    context = self.load_context(
                        clean_data_paths, input_datatype)
                    print(data_type)
                    print(context[0][:100])
                else:
                    context = context_data
                tfidf_transform[data_type] = TfidfVectorizer(
                    input='content', ngram_range=ngram_range, token_pattern=r"(?u)\b[\w\d]+\b")
                context_matrix[data_type] = tfidf_transform[data_type].fit_transform(
                    context).tocsr()
                ...
                scipy.sparse.save_npz(os.path.join(
                    save_tfids_object_path, f'sparse_context_matrix_{data_type}.npz'), context_matrix[data_type])
            ...
            for cxx_context_path in context_paths:
                ...
                for path in paths:
                    with open(path, encoding='utf-8') as f:
                        context += [self.preprocess_text(' '.join(line))
...
class object_retrieval(load_file):
    def __init__(
            ...
            update: bool = False,
            save_tfids_object_path=os.path.join(
                PROJECT_ROOT, 'dict/bin/contexts_bin'),
        ...
        all_datatpye = [key for key in clean_data_path.keys() if (
            clean_data_path[key] != None)]
        super().__init__(
        ...
        for data_type in all_datatpye:
            ...
            self.context_matrix[data_type] = scipy.sparse.load_npz(os.path.join(
    ...
    def transform_input(
        ...
        if transform_type in ['bbox', 'class', 'color', 'tag', 'number']:
            vectorize = self.tfidf_transform[transform_type].transform([
                                                                       input_query])
        else:
